...
import { addJob, getJob, removeJob, updateJob, markApplicationRead, removeApplication, getApplication, type JobStatus } from "@/lib/jobs";
...
// Refus courtois (Alfred le rédige si possible) — logique partagée avec Telegram.
export async function sendRejectionAction(formData: FormData) {
  ...
  revalidatePath(PATH);
  redirect(`${PATH}?refus=${r.ok ? "ok" : "err"}`);
}

// Invitation à l'entretien (2e étape), avec lien Calendly si configuré.
export async function sendInviteAction(formData: FormData) {
  ...
  revalidatePath(PATH);
  redirect(`${PATH}?invite=${r.ok ? "ok" : "err"}`);
}
